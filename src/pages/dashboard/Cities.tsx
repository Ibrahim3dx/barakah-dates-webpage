...
import { Plus, Edit, Trash2, Search, CheckSquare, Square, Upload, Download } from 'lucide-react';
import CityForm from '@/components/dashboard/CityForm';
import api from '@/lib/api';
import { City, CitiesResponse } from '@/types/dashboard';
...
const Cities = () => {
  ...
  const { data: cities, isLoading } = useQuery<CitiesResponse>({
    queryKey: ['cities', searchQuery, currentPage, pageSize],
    queryFn: async () => {
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: async ({ id, is_active }: { id: number; is_active: boolean }) => {
      const response = await api.patch(`/api/cities/${id}/status`, { is_active });
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cities'] });
    ...
    mutationFn: async (is_active: boolean) => {
      await Promise.all(
        selectedCities.map(id =>
          api.patch(`/api/cities/${id}/status`, { is_active })
        )
      );
    },
    onSuccess: () => {
  ...
  const handleDownloadSample = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/cities/import/sample`, {
        headers: {
  ...
  const handleImportChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await api.post('/api/cities/import', formData, {
      ...
      if (response.data?.summary) {
        ...
        const message = `Import completed: ${created} created, ${updated} updated`;
        alert(message);
      } else {
      ...
      if (errorData?.missing_columns) {
        ...
        const detailedMessage = `Missing required columns in CSV file\n\n` +
          `Missing columns: ${missingCols}\n` +
          `Found columns: ${foundCols}\n` +
          `Required columns: ${requiredCols}\n\n` +
          `${errorData.help || 'Please download the sample CSV file to see the correct format'}`;
        ...
        alert(`Import failed: ${errorData?.message || (error as Error)?.message || 'Unknown error'}`);
      }
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div className="flex items-center gap-4 flex-wrap">
          <h1 className="text-2xl font-semibold text-gray-900">{t('dashboard.cities.title') || 'Cities'}</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Download className="h-4 w-4 mr-1" /> {t('dashboard.cities.download_sample') || 'Sample CSV'}
            </button>
            <button
              type="button"
            ...
            >
              <Upload className="h-4 w-4 mr-1" /> {isImporting ? (t('dashboard.cities.importing') || 'Importing...') : (t('dashboard.cities.import_csv') || 'Import CSV')}
            </button>
            <input
              ref={fileInputRef}
              type="file"
          ...
          {selectedCities.length > 0 && (
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-600">
                {selectedCities.length} {t('dashboard.cities.selected') || 'selected'}
              </span>
              <div className="flex gap-2">
                ...
                >
                  {t('dashboard.cities.batch_activate') || 'Activate'}
                </button>
                <button
                ...
                >
                  {t('dashboard.cities.batch_deactivate') || 'Deactivate'}
                </button>
                <button
        ...
        >
          ...
          {t('dashboard.cities.add_city') || 'Add City'}
        </button>
      </div>
        ...
        <div className="max-w-lg w-full">
          <div className="relative">
            ...
            <input
              ...
              placeholder={t('dashboard.cities.search_placeholder') || 'Search cities...'}
              value={searchQuery}
              ...
              {cities?.data?.map((city: City) => (
                <tr key={city.id} className={selectedCities.includes(city.id) ? 'bg-indigo-50' : ''}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {formatCurrency(city.delivery_price)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <button
                    ...
                    >
                      {city.is_active ? (t('dashboard.cities.active') || 'Active') : (t('dashboard.cities.inactive') || 'Inactive')}
                    </button>
                  </td>
        ...
        {cities && cities.last_page > 1 && (
          <div className="bg-white px-4 py-3 border-t border-gray-200 flex items-center justify-between">
            <div className="flex-1 flex justify-between sm:hidden">
              ...
              <button
                onClick={() => setCurrentPage(Math.min(cities.last_page, currentPage + 1))}
                disabled={currentPage >= cities.last_page}
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">
                    {Math.min(currentPage * pageSize, cities.total)}
                  </span>
                  {' '}{t('common.of') || 'of'}{' '}
                  <span className="font-medium">{cities.total}</span>
              ...
              <div>
                <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                  ...
                  {/* Page Numbers */}
                  {Array.from({ length: Math.min(5, cities.last_page) }, (_, i) => {
                    const startPage = Math.max(1, Math.min(cities.last_page - 4, currentPage - 2));
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mt-4">
                {t('dashboard.cities.delete_confirmation') || 'Delete City'}
              </h3>
              <div className="mt-2 px-7 py-3">
                <p className="text-sm text-gray-500">
                  {t('dashboard.cities.delete_confirmation_message') || 'Are you sure you want to delete'} <strong>"{cityToDelete.name}"</strong>? {t('dashboard.cities.delete_warning') || 'This action cannot be undone.'}
                </p>
              </div>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" dir={isRTL ? 'rtl' : 'ltr'}>
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mt-4">
                {t('dashboard.cities.batch_delete_confirmation') || 'Delete Cities'}
              </h3>
              <div className="mt-2 px-7 py-3">
                <p className="text-sm text-gray-500">
                  {t('dashboard.cities.batch_delete_message') || 'Are you sure you want to delete'} <strong>{selectedCities.length}</strong> {t('dashboard.cities.batch_delete_cities') || 'cities'}? {t('dashboard.cities.delete_warning') || 'This action cannot be undone.'}
                </p>
              </div>
              <div className="flex justify-center gap-4 mt-4">
                <button
